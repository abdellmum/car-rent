...
<body>
    <div class="wrapper mt-sm-5">
        ...
        ?>
        <h4 class="pb-4 border-bottom text-center">Conexion a votre compte</h4>
        <form action="<?php echo base_url().'login/authenticate' ;?>" name="loginform" id="loginform" method="POST">
            ...
            <div class="py-3 pb-4 border-bottom">
                <button type="submit" class="btn btn-success mr-3">Se connecter</button>
                <a href="<?php echo base_url().'singup/index' ?>" class="btn btn-danger">s'enregister</a>
            </div>
        </form>
    </div>
    <script>
    ...
    const validate = () => {
        ...
        if (usernameVal === "") {
            setErrorMsg(username, 'udonnées incorrectes');
        } else {
            setSuccessMsg(username);
        }

        if (passwordVal === "") {
            setErrorMsg(password, 'données incorrectes');
        } else {
